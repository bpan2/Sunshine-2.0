...
import com.spreys.sunshine.app.MainActivity;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
        ...
        if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
            ...
            if (cursor.moveToFirst()) {
                ...
                boolean notificationsEnabled = prefs.getBoolean(
                        context.getString(R.string.pref_general_notifications_key),
                        true
                );
    ...
    private String formatHighLows(double high, double low) {
        ...
        String unitType = sharedPrefs.getString(
                getContext().getString(R.string.pref_general_units_key),
                getContext().getString(R.string.pref_general_units_metric_value));

        if (unitType.equals(getContext().getString(R.string.pref_general_units_imperial_value))) {
            high = (high * 1.8) + 32;
            low = (low * 1.8) + 32;
        } else if (!unitType.equals(getContext().getString(R.string.pref_general_units_metric_value))) {
    ...
    private void getWeatherDataFromJson(String forecastJsonStr, int numDays, String locationSetting)
        ...
        final String OWN_RESPONSE_CODE = "cod";

        JSONObject forecastJson = new JSONObject(forecastJsonStr);

        //Check location was valid
        if (forecastJson.getString(OWN_RESPONSE_CODE).equals("404")) {
        ...
        int insertedValues = getContext().getContentResolver().bulkInsert(
    ...
    private long addLocation(String locationSetting, String cityName, double cityLatitude, double cityLongitude) {
        ...
        if (locationCursor.moveToFirst()) {
            Log.v(TAG, "Found it in the database!");
            int locationIdIndex = locationCursor.getColumnIndex(WeatherContract.LocationEntry._ID);
            return locationCursor.getLong(locationIdIndex);
        } else {
            Log.v(TAG, "Didn't find it in the database, inserting now!");
